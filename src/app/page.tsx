...
export default function LandingPage() {
  ...
  const cards = [
    { icon: '📅', title: 'Événements', desc: 'Inscrivez-vous aux soirées, dîners et célébrations.', href: '/events' },
    { icon: '🍽️', title: 'Galas & tables', desc: 'Réservez une table complète ou des places.', href: '/events' },
    { icon: '🤲', title: 'Faire un don', desc: 'Soutenez la communauté, reçu fiscal immédiat.', href: '/don' },
    { icon: '🕯️', title: 'Horaires Chabbat', desc: 'Bougies, havdalah et zmanim pour Lyon.', href: '/horaires' },
  ];
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-5 sm:px-10 py-3 sm:py-4">
        ...
        <motion.nav aria-label="Navigation" className="relative flex items-center gap-2 sm:gap-3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }}>
          <Link href="/events" className="tap-target hidden sm:inline-flex items-center px-3 py-2 text-sm text-brown-600 hover:text-brown-900 transition-colors">Événements</Link>
          {user ? (
            <>
              <Link href="/compte" aria-label="Mon compte">
                <span className="tap-target inline-flex items-center px-3.5 py-2 rounded-xl bg-gold-400/10 border border-gold-400/30 text-gold-700 text-sm font-medium hover:bg-gold-400/20 transition-colors">Mon compte</span>
              </Link>
              <button onClick={handleLogout} className="tap-target px-2.5 py-2 text-sm text-brown-500 hover:text-brown-800 transition-colors">Déconnexion</button>
            </>
          ) : (
            <Link href="/user-login" aria-label="Se connecter">
              <span className="tap-target inline-flex items-center px-4 py-2 rounded-xl bg-brown-900 text-beige-50 text-sm font-medium">Se connecter</span>
            </Link>
          )}
          <Link href="/login" className="tap-target inline-flex items-center ml-0.5 text-xs text-brown-400 hover:text-brown-700 transition-colors" aria-label="Administration">Admin</Link>
        </motion.nav>
      </header>
      ...
      <main id="contenu" className="flex-1 flex flex-col items-center justify-center px-5 sm:px-6 pt-28 sm:pt-24 pb-16 text-center relative">
        ...
        <motion.div className="relative z-10 max-w-2xl" initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] }}>
          ...
          <h1 className="text-[clamp(2.5rem,8vw,6rem)] font-light text-brown-900 leading-[1.05] mb-5" style={{ fontFamily: 'var(--font-cormorant)' }}>
            Bienvenue chez <em className="gradient-text not-italic">HabadLyon</em>
          </h1>
          <p className="text-brown-600 text-base sm:text-xl leading-relaxed mb-9 max-w-lg mx-auto">
            Inscrivez-vous aux événements, réservez votre table de gala, faites un don —
        ...
        {!user && (
          <motion.div className="relative z-10 mt-16 w-full max-w-3xl rounded-3xl bg-brown-900 text-beige-50 p-8 sm:p-10 text-center overflow-hidden"
            ...
            <h2 className="text-2xl sm:text-3xl font-light mb-2" style={{ fontFamily: 'var(--font-cormorant)' }}>Votre espace membre</h2>
            <p className="text-beige-300 text-sm mb-6 max-w-md mx-auto">Créez un compte pour retrouver toutes vos inscriptions, vos billets et vos reçus au même endroit.</p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
              <Link href="/register" className="w-full sm:w-auto">
                <span className="tap-target inline-flex items-center justify-center w-full sm:w-auto px-8 py-3.5 rounded-xl bg-gold-400 text-brown-900 font-semibold text-sm hover:bg-gold-300 transition-colors">Créer mon compte</span>
              </Link>
              <Link href="/user-login" className="w-full sm:w-auto">
                <span className="tap-target inline-flex items-center justify-center w-full sm:w-auto px-8 py-3.5 rounded-xl border border-beige-50/30 text-beige-50 font-medium text-sm hover:bg-beige-50/10 transition-colors">J&apos;ai déjà un compte</span>
              </Link>
            </div>
            <p className="text-beige-400/60 text-xs mt-4">Pas obligatoire : vous pouvez aussi vous inscrire à un événement sans compte.</p>
          </motion.div>
        )}
      </main>

      {/* Bande communautés (SaaS) */}
      <section className="px-5 sm:px-6 py-12 border-t border-beige-200">
        <div className="max-w-3xl mx-auto text-center">
          <p className="text-xs uppercase tracking-widest text-gold-600 mb-2">Vous gérez une communauté ?</p>
          <h2 className="text-2xl sm:text-3xl font-light text-brown-900 mb-3" style={{ fontFamily: 'var(--font-cormorant)' }}>
            Créez l&apos;espace en ligne de votre communauté
          </h2>
          <p className="text-brown-500 text-sm mb-6 max-w-lg mx-auto">
            Événements, réservations de gala, dons avec reçus fiscaux, billets QR, horaires de Chabbat — votre propre site, prêt en quelques minutes.
          </p>
          <Link href="/signup">
            <motion.span className="btn-liquid inline-flex items-center px-8 py-3.5 rounded-xl bg-brown-900 text-beige-50 font-medium text-sm overflow-hidden"
              ...
              <span className="relative z-10">Créer mon espace gratuit →</span>
            </motion.span>
          </Link>
        </div>
      </section>
      ...
      <footer className="py-6 text-center safe-bottom border-t border-beige-200">
        ...
        <Link href="/infos" className="text-xs text-brown-300 hover:text-brown-600 transition-colors mt-1 inline-block">Mentions légales · Contact</Link>
      </footer>
    </div>
  );
}
